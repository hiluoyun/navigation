...
bool MySetLayer::init()
{
	...
	item2 = CCMenuItemToggle::createWithTarget(this,menu_selector(MySetLayer::menucallback1),ms1,ms2,NULL);
	...
	CCMenuItemToggle *item3 = CCMenuItemToggle::createWithTarget(this,menu_selector(MySetLayer::menucallback2),sp6,sp7,sp8,NULL);
	item3->setPosition(ccp(sp1->getContentSize().width*0.64,sp1->getContentSize().height*0.2));
	item3->setScale(0.8);
	if(CCUserDefault::sharedUserDefault()->getIntegerForKey("hard")==1)
	{
		item3->setSelectedIndex(0);
	}
	else if(CCUserDefault::sharedUserDefault()->getIntegerForKey("hard")==2)
	{
		item3->setSelectedIndex(1);
	}
	else
	{
		item3->setSelectedIndex(2);
	}
...
//空的函数返回
void MySetLayer::menucallback1(CCObject *sp6)
{
	if(CCUserDefault::sharedUserDefault()->getBoolForKey(EFFECT))
	{
...
void MySetLayer::menucallback2(CCObject *sp7)
{
	if(CCUserDefault::sharedUserDefault()->getIntegerForKey("hard")==1)
	{
		CCUserDefault::sharedUserDefault()->setIntegerForKey("hard",2);
	}
	else if(CCUserDefault::sharedUserDefault()->getIntegerForKey("hard")==2)
	{
		CCUserDefault::sharedUserDefault()->setIntegerForKey("hard",3);
	}
	else
	{
